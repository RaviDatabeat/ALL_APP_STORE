    ...
    async def process(self, input_path: Path):
        ...
        try:
            ...
            if not self.lg_file_path.exists():
                ...
                failure_data = [
                    {"bundle_id": bid, "error": "LG validation file not found"}
                    for bid in input_ids
                ]
                Path("failure_output").mkdir(exist_ok=True)
                self.replace_to_parquet(
                    failure_data, Path("failure_output/lg_failure.parquet")
                )
                return
            ...
            #     Path("failure_output").mkdir(exist_ok=True)
            #     self.replace_to_parquet(
            #         self.failure_data_list, Path("failure_output/lg_failure.parquet")
            #     )
            #     logger.info(f"Saved {len(self.failure_data_list)} failures to failure_output/lg_failure.parquet")

            # logger.info(
            #     f"LG processing complete. Success: {len(self.output_data_list)}, Failures: {len(self.failure_data_list)}"
            # )
